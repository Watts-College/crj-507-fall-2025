...
4. Plot all three networks. Use the `edge_rescale()` function in the `SNACpack` package to weight the edges for each projected graph.
...
12. If we were examining degree centrality, what would a completely centralized network look like? What would its degree centrality scores be? Describe it (or, if your feeling feisty, create one and plot it!).
13. If we were examining betweenness centrality, what would a completely decentralized network look like? What would its betweenness centrality scores be? Describe it (or, if your still feeling feisty, create one and plot it!).
...
For the *person* matrix you created in step 2, repeat steps 5-16 then answer: 
